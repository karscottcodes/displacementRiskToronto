...
function InfoPanel({ info }) {
	...
	return (
		<div>
			{info.area_name ? (
				<>
					<h2 className="text-2xl pb-3">{info.area_name}</h2>
					<p className="pb-3">Designation: {info.designation}</p>
					<div>
						...
						<p className="pb-3">
							Unemployment Rate (%):{" "}
							{info.VULNERABILITIES.unemployment_rate}
						</p>
						<p className="pb-3">Rent Burdened (Greater than 30% of Income) (%): {info.VULNERABILITIES.rent_burdened}</p>
						<p>Canadian Citizen: {info.VULNERABILITIES.can_citizen}</p>
						<p>Not Canadian Citizen: {info.VULNERABILITIES.not_can_citizen}</p>
						<h4 className="font-bold py-3">
							Income Factors
						</h4>
						<p>Avg Income ($ CDN): {info.VULNERABILITIES.inc_average}</p>
						<p>Income in Lower Half: {info.VULNERABILITIES.inc_lower_half}</p>
						<p>Income in Upper Half: {info.VULNERABILITIES.inc_upper_half}</p>
						<h4 className="font-bold py-3">
							Population Distribution
						</h4>
						<p>% of Pop 0-14: {info.VULNERABILITIES.zero_fourteen}</p>
						<p>% of Pop 15-64: {info.VULNERABILITIES.fifteen_sixtyfour}</p>
						<p>% of Pop 65+: {info.VULNERABILITIES.sixtyfiveplus}</p>
						<p>% of Pop 85+: {info.VULNERABILITIES.eightyfiveplus}</p>
					</div>
					<div>
						<h3 className="text-xl py-3 font-bold">Housing Conditions</h3>
						<h4 className="font-bold py-3">
							Housing Type
						</h4>
						<p>Single Detached: {info.CONDITIONS.single_detached}</p>
						...
						<p>Apt (Less than 5 Floors): {info.CONDITIONS.apt_less_5}</p>
						<p>Apt (More than 5 Floors): {info.CONDITIONS.apt_more_5}</p>
					...
					<div>
						<h3 className="text-xl py-3 font-bold">Market Pressures</h3>
						<p>Median Rent ($ CDN): {info.PRESSURES.median_rent}</p>
						<p>Average Rent ($ CDN): {info.PRESSURES.average_rent}</p>
					</div>
				</>
			) : (
				welcomeMsg
